...
def _merge_rule_to_line(rule):
    """Hugging Face BPE: merge is either 'a b' string or ['a', 'b'] (e.g. Qwen3)."""
    if isinstance(rule, str):
        return rule
    if isinstance(rule, (list, tuple)):
        return " ".join(str(part) for part in rule)
    raise TypeError(f"Unexpected merge rule type: {type(rule)!r}")
...
def export_merges_txt(json_config, output_dir):
    ...
    with open(merges_txt_path, 'ab') as f:
        ...
        for rule in json_config['model']['merges']:
            line = _merge_rule_to_line(rule)
