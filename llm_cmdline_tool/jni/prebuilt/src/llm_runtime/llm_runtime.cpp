...
bool LlmRuntime::initialize(const LlmModelOptions& modelOptions,
    ...
    // Cache eviction models use 4 rotemb inputs
    const auto useCacheEviction = runtimeOptions.cacheEvictionMode != CacheEvictionMode::None;
    const size_t numRotEmbInputs = useCacheEviction ? 4 : modelOptions.rotEmbNumInputs;
    ...
    for (size_t chunkIdx = 0; chunkIdx < numChunks; ++chunkIdx) {
        ...
        auto dlaExec = llmExecFactory.create<LlmExecutor>(
            ...
            mRotEmbMasterLut.get(), numRotEmbInputs, getLoraWeightsfileMap(chunkIdx),
            getCacheFile(chunkIdx));
