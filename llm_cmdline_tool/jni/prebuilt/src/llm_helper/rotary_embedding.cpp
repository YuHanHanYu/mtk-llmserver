...
public:
    explicit LookupHelper(const RotaryEmbeddingMasterLut& rotEmbMasterLut,
        ...
        : kRowSizeBytes(2 * rotEmbMasterLut.kHeadDim * rotEmbMasterLut.kTypeSize),
          ...
          kSingleEmbSize((kRowSizeBytes / 2) / (rotEmbMasterLut.kDuplicateSinCos ? 1 : 2)),
...
RotaryEmbeddingMasterLut::RotaryEmbeddingMasterLut(const LLMType rotEmbType, const size_t length,
                                                   ...
                                                   const bool duplicateSinCos, const float ntkScale)
    : kType(rotEmbType),
      kTypeSize(getLLMTypeSize(kType)),
      kLength(length),
      kHeadDim(headDim),
      kRotBase(rotBase),
      kDuplicateSinCos(duplicateSinCos),
      kNtkScale(ntkScale) {
...
void RotaryEmbeddingMasterLut::generate() {
    ...
    for (int pos = 0; pos < kLength; pos++) { // row in lut
        for (int dim = 0; dim < rotDimHalf; dim++) {
            ...
            //                    rotDim     |
            //                        rotDim + rotDimHalf
            // During setEmbed, the duplicated part will be ignored if kDuplicateSinCos is false.
...
// RotaryEmbeddingMasterLut supports 1 or 2 or 4 rotary embedding inputs
void RotaryEmbeddingMasterLut::setEmbed(std::vector<void*> rotEmbedBuffers, const size_t tokenIndex,
                                        ...
                                        const size_t modelCacheSize) const {
    ...
    switch (numRotEmbInputs) {
        ...
        case 4: {
            const auto rotEmbQCosInput = rotEmbedBuffers[0];
            const auto rotEmbQSinInput = rotEmbedBuffers[1];
            const auto rotEmbKCosInput = rotEmbedBuffers[2];
            const auto rotEmbKSinInput = rotEmbedBuffers[3];
            // The 4 rot emb inputs variant requires cache size information
            setEmbed(rotEmbQCosInput, rotEmbQSinInput, rotEmbKCosInput, rotEmbKSinInput, tokenIndex,
                     modelTokenSize, modelCacheSize, leftPadLength, rightPadLength);
            break;
        }
        default:
...
void RotaryEmbeddingMasterLut::setEmbed(void* rotEmbQCosBuffer, void* rotEmbQSinBuffer,
                                        void* rotEmbKCosBuffer, void* rotEmbKSinBuffer,
                                        const size_t tokenIndex, const size_t modelTokenSize,
                                        const size_t modelCacheSize, const size_t leftPadLength,
                                        const size_t rightPadLength) const {
    // Generate Master Lut if not yet done
    if (!mIsReady) {
        LOG(ERROR) << "Attempting to use the rotary embedding lookup table before being "
    ...
    // The the master lut stores in [tokenIdx, 2, kHeadDim].
    // Whereas the model takes in the rot emb in this order:
    //   - Query Cos: [modelTokenSize             , kHeadDim]
    //   - Query Sin: [modelTokenSize             , kHeadDim]
    //   -  Key  Cos: [modelTokenSize + modelCacheSize, kHeadDim]
    //   -  Key  Sin: [modelTokenSize + modelCacheSize, kHeadDim]

    DCHECK_LE(modelTokenSize, modelCacheSize) << "Model token batch size > cache size.";
    DCHECK_GE(modelTokenSize, leftPadLength + rightPadLength)
        << "Padding size exceeds model input token size.";
    DCHECK(leftPadLength == 0 || tokenIndex == 0) << "Left pad is used with token index > 0.";

    const auto padLength = leftPadLength + rightPadLength;

    //===----------------------------------------===//
    // RotEmb for Query: [modelTokenSize, kHeadDim]
    //===----------------------------------------===//

    const auto startTokenIndexQ = std::min(tokenIndex, modelCacheSize);
    const size_t numQLookup = modelTokenSize - padLength;

    LookupHelper lookupHelperQ(*this, startTokenIndexQ);
    lookupHelperQ.setLookup(numQLookup);

    // Lookup cos
    lookupHelperQ.setTarget(rotEmbQCosBuffer);
    lookupHelperQ.pad(leftPadLength);
    lookupHelperQ.lookupCos();
    lookupHelperQ.pad(rightPadLength);

    // Lookup sin
    lookupHelperQ.setTarget(rotEmbQSinBuffer);
    lookupHelperQ.pad(leftPadLength);
    lookupHelperQ.lookupSin();
    lookupHelperQ.pad(rightPadLength);

    //===---------------------------------------------------===//
    // RotEmb for Key: [modelCacheSize + modelTokenSize, kHeadDim]
    //===---------------------------------------------------===//
    const size_t startTokenIndexK = 0; // Start lookup from top
    const size_t numKLookup =
        std::min(modelTokenSize + modelCacheSize, modelTokenSize + tokenIndex - padLength);

    const auto leftPadLengthK =
        modelCacheSize - std::min(modelCacheSize, tokenIndex) + leftPadLength;

    LookupHelper lookupHelperK(*this, startTokenIndexK);
    lookupHelperK.setLookup(numKLookup);

    // Lookup cos
    lookupHelperK.setTarget(rotEmbKCosBuffer);
    lookupHelperK.pad(leftPadLengthK);
    lookupHelperK.lookupCos();
    lookupHelperK.pad(rightPadLength);

    // Lookup sin
    lookupHelperK.setTarget(rotEmbKSinBuffer);
    lookupHelperK.pad(leftPadLengthK);
    lookupHelperK.lookupSin();
    lookupHelperK.pad(rightPadLength);
}
...
void RotaryEmbeddingMasterLut::setEmbed(std::vector<void*> rotEmbedBuffers, const size_t tokenIndex,
                                        ...
                                        const size_t tokenOffset, const size_t leftPadLength,
                                        const size_t rightPadLength) const {
    const auto numRotEmbInputs = rotEmbedBuffers.size();
    switch (numRotEmbInputs) {
        case 1: {
            const auto rotEmbInput = rotEmbedBuffers[0];
            setEmbed(
                rotEmbInput, tokenIndex, positions, tokenOffset, leftPadLength, rightPadLength);
            break;
        }
        case 2: {
            const auto rotEmbCosInput = rotEmbedBuffers[0];
            const auto rotEmbSinInput = rotEmbedBuffers[1];
            setEmbed(rotEmbCosInput, rotEmbSinInput, tokenIndex, positions, tokenOffset,
                     leftPadLength, rightPadLength);
            break;
        }
        default:
            LOG(FATAL) << "RotaryEmbeddingMasterLut: Unsupported number of rotary embedding inputs "
...
void RotaryEmbeddingMasterLut::setEmbed(void* rotEmbedBuffer, const size_t tokenIndex,
    ...
    // Calculate real tokenIndex for beam search
    size_t realTokenIndex = tokenIndex - tokenOffset;
    const auto requestedMaxIndex =
        realTokenIndex + *std::max_element(positions.begin(), positions.end());
    ...
    LookupHelper lookupHelper(*this, realTokenIndex);
...
void RotaryEmbeddingMasterLut::setEmbed(void* rotEmbedCosBuffer, void* rotEmbedSinBuffer,
    ...
    // Calculate real tokenIndex for beam search
    size_t realTokenIndex = tokenIndex - tokenOffset - leftPadLength;
    const auto requestedMaxIndex =
        realTokenIndex + *std::max_element(positions.begin(), positions.end());
    const auto availableLength = getRotEmbedLength();
    if (requestedMaxIndex >= availableLength) {
