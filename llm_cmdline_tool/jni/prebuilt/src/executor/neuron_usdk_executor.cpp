...
void NeuronUsdkExecutor::createNeuronMemory(IOBuffer& ioBuf) {
    ...
    if (ioBuf.ahwbHandle != nullptr) {
        mLib->NeuronMemory_createFromAHardwareBuffer(ioBuf.ahwbHandle, &ioBuf.neuronMemory);
    } else if (ioBuf.fd != -1) {
        mLib->NeuronMemory_createFromFd(
...
std::unique_ptr<Allocator> NeuronUsdkExecutor::createMemoryAllocator() {
    return std::make_unique<AhwBufferAllocator>();
}
...
bool NeuronUsdkExecutor::loadDla(const void* buffer, const size_t size, UsdkRuntime* runtime) {
    ...
    int err = mLib->NeuronModel_create(&model);
    ...
    for (int i = 0; i < getNumInputs(); i++) {
        ...
        err |= mLib->NeuronModel_addOperand(model, &tensorInputType);
        inputNode.emplace_back(i);
    }

    auto getDlaExtOperandType = [this] {
        NeuronRuntimeVersion version;
        mLib->Neuron_getVersion(&version);
    ...
    err |= mLib->NeuronModel_getExtensionOperandType(
    ...
    err |= mLib->NeuronModel_addOperand(model, &extenOperandType);
    ...
    for (int i = 0; i < getNumOutputs(); i++) {
        ...
        err |= mLib->NeuronModel_addOperand(model, &tensorOutputType);
    ...
    err |= mLib->NeuronModel_setOperandValue(model, inputNode.back(), buffer, size);

    int32_t operationType = 0;
    const uint16_t network_operation_type_restore = RESTORE_DLA_EXTENSION_OPERATION_TYPE;
    err |= mLib->NeuronModel_getExtensionOperationType(
    ...
    err |= mLib->NeuronModel_addOperation(model, static_cast<NeuronOperationType>(operationType),
    ...
    err |= mLib->NeuronModel_identifyInputsAndOutputs(
        model, inputNode.size() - 1, inputNode.data(), outputNode.size(), outputNode.data());

    err |= mLib->NeuronModel_finish(model);
    ...
    if (mLib->NeuronCompilation_createWithOptions(model, &compilation, kOptions.c_str())
        != NEURON_NO_ERROR) {
        LOG(ERROR) << "NeuronCompilation_create fail";
        return false;
    };

    mLib->NeuronCompilation_setPriority(compilation, NEURON_PRIORITY_HIGH);
    mLib->NeuronCompilation_setPreference(compilation, NEURON_PREFER_SUSTAINED_SPEED);

    if (!kOptions.empty()) {
        mLib->NeuronCompilation_setOptimizationString(compilation, kOptions.c_str());
    }
    ...
    // So init without weight sharing will not get much benefit from mult-threading.
    {
        ...
        if (mLib->NeuronCompilation_finish(compilation) != NEURON_NO_ERROR) {
            LOG(ERROR) << "NeuronCompilation_finish fail";
            return false;
        };
    }

    if (mLib->NeuronExecution_create(compilation, &execution) != NEURON_NO_ERROR) {
        LOG(ERROR) << "NeuronExecution_create fail";
        return false;
    };
    if (mLib->NeuronExecution_setBoostHint(execution, 100) != NEURON_NO_ERROR) {
        LOG(ERROR) << "NeuronExecution_setBoostHint fail";
        return false;
    };

#ifndef NO_DISABLE_SYNC_INPUT
    if (mLib->NeuronExecution_setCacheFlushHint(execution, NEURON_CACHE_FLUSH_DISABLE_SYNC_INPUT)
        != NEURON_NO_ERROR) {
        LOG(ERROR) << "NeuronExecution_setCacheFlushHint fail";
        return false;
    }
#endif
...
void NeuronUsdkExecutor::runInferenceImpl() {
    ...
    mLib->NeuronExecution_compute(getUsdkRuntime()->execution);
}
...
void* NeuronUsdkExecutor::createRuntime(FileSource modelFile) {
    ...
    const auto [dlaBuffer, dlaSize] = modelFile.get();

    // Create runtime
    auto runtime = std::make_unique<UsdkRuntime>();
...
void NeuronUsdkExecutor::releaseRuntime(void* runtime) {
    ...
    if (execution != nullptr) {
        mLib->NeuronExecution_free(execution);
    }
    if (compilation != nullptr) {
        mLib->NeuronCompilation_free(compilation);
    }
    if (model != nullptr) {
        mLib->NeuronModel_free(model);
    }
    delete usdkRuntime;
}
...
void NeuronUsdkExecutor::release() {
    for (auto neuronMemory : mCreatedNeuronMems) {
        mLib->NeuronMemory_free(neuronMemory);
    }
...
#define NEURON_USDK_SET_INPUT(inputIdx, ioBuf, size)                                          \
    mLib->NeuronExecution_setInputFromMemory(getUsdkRuntime()->execution, inputIdx,           \
                                             (NeuronOperandType*)NULL, ioBuf.neuronMemory, 0, \
                                             size);
...
#define NEURON_USDK_SET_OUTPUT(outputIdx, ioBuf, size)                                         \
    mLib->NeuronExecution_setOutputFromMemory(getUsdkRuntime()->execution, outputIdx,          \
                                              (NeuronOperandType*)NULL, ioBuf.neuronMemory, 0, \
                                              size);
...
void NeuronUsdkExecutor::setRuntimeOffsetedInput(const size_t index, const size_t offset) {
    ...
    mLib->NeuronExecution_setInputFromMemory(getUsdkRuntime()->execution, index,
                                             (NeuronOperandType*)NULL, ioBuf.neuronMemory, offset,
                                             ioBuf.usedSizeBytes);
}
...
void NeuronUsdkExecutor::setRuntimeOffsetedOutput(const size_t index, const size_t offset) {
    ...
    mLib->NeuronExecution_setOutputFromMemory(getUsdkRuntime()->execution, index,
                                              (NeuronOperandType*)NULL, ioBuf.neuronMemory, offset,
                                              ioBuf.usedSizeBytes);
}
...
size_t NeuronUsdkExecutor::getRuntimeNumInputs() const {
    ...
    size_t size;
    while (true) {
        if (mLib->NeuronCompilation_getInputPaddedSize(compilation, numInputs, &size)
            == NEURON_BAD_DATA)
            break;
        numInputs++;
    }
...
size_t NeuronUsdkExecutor::getRuntimeNumOutputs() const {
    ...
    size_t size;
    while (true) {
        if (mLib->NeuronCompilation_getOutputPaddedSize(compilation, numOutputs, &size)
            == NEURON_BAD_DATA)
            break;
        numOutputs++;
    }
...
size_t NeuronUsdkExecutor::getRuntimeInputSizeBytes(const size_t index) const {
    ...
    auto compilation = getUsdkRuntime()->compilation;
    mLib->NeuronCompilation_getInputPaddedSize(compilation, index, &inputSizeBytes);

    // clang-format off
    uint32_t dims[4];
    mLib->NeuronCompilation_getInputPaddedDimensions(compilation, index, dims);
...
size_t NeuronUsdkExecutor::getRuntimeOutputSizeBytes(const size_t index) const {
    ...
    auto compilation = getUsdkRuntime()->compilation;
    mLib->NeuronCompilation_getOutputPaddedSize(compilation, index, &outputSizeBytes);

    // clang-format off
    uint32_t dims[4];
    mLib->NeuronCompilation_getOutputPaddedDimensions(compilation, index, dims);
...
void NeuronUsdkExecutor::getRuntimeInputShape(const size_t index, uint32_t* shape) const {
    ...
    mLib->NeuronCompilation_getInputPaddedDimensions(
        getUsdkRuntime()->compilation, index, tmpShape);
...
void NeuronUsdkExecutor::getRuntimeOutputShape(const size_t index, uint32_t* shape) const {
    uint32_t tmpShape[kDimensionSize];
    mLib->NeuronCompilation_getOutputPaddedDimensions(
        getUsdkRuntime()->compilation, index, tmpShape);
    std::memcpy(shape, tmpShape, sizeof(tmpShape));
}

void NeuronUsdkExecutor::loadAdapterLib() {
    mLib = NeuronAdapterLib::Get();
    if (mLib == nullptr) {
        LOG(ERROR) << "Couldn't dlopen the Neuron Adapter library.";
        std::abort();
    }
}
