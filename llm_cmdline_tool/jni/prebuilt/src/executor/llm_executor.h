...
public:
    ...
    explicit LlmExecutor(const std::vector<RuntimeInfo>& runtimeInfos,
        ...
        : ExecutorBackend(getModelFiles(runtimeInfos), sharedWeights),
          ...
          kCacheEvictionMode(runtimeOptions.cacheEvictionMode),
          kSnapKvAttnSinkSize(runtimeOptions.localSnapKvAttnSinkSize) {}
...
private:
    size_t getExpectedNumInputs() const { return mExpectedNumInputs; }
    size_t getExpectedNumOutputs() const { return mExpectedNumOutputs; }
...
protected:
    ...
    const CacheEvictionMode kCacheEvictionMode = CacheEvictionMode::None;
    const size_t kSnapKvAttnSinkSize = 0;
