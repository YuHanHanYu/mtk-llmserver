...
public:
    ...
    // IO dumping
    virtual bool saveInputs(const std::string& directory, const std::string& name) const;
    virtual bool saveOutputs(const std::string& directory, const std::string& name) const;
...
protected:
    ...
    virtual void preInitBufferProcess() { verifyBatchSize(); };
    ...
    // Verify batch size
    void verifyBatchSize() const;

    // Get the batch dim of first input. Used by verifyBatchSize().
    virtual size_t getInputBatchDim() const { return 0; };

    // Duplicate batch 0 to all batches for given Input/Output
    void inputDupAllBatches(const size_t index, const void* src = nullptr);
    void outputDupAllBatches(const size_t index, const void* src = nullptr);
