...
void LlmRingBufferExecutor::runInferenceEpilogue() {
    ...
    if (mCacheUpdateMode == CacheUpdateMode::Copy) {
        // Cache Eviction
        // Run before cache append and ring offset advancement.
        runCacheEviction();
...
bool LlmRingBufferExecutor::saveInputs(const std::string& directory,
    ...
    const auto& inputCacheIdxs = this->getCacheInputIdxs();
    std::unordered_set<size_t> inputCacheIdxSet(inputCacheIdxs.begin(), inputCacheIdxs.end());

    auto getBufferData = [&](const size_t inputIdx) -> std::pair<char*, size_t> {
        const auto input = getInput(inputIdx);
        const auto buffer = reinterpret_cast<char*>(input.buffer);
        const auto size = input.usedSizeBytes;
        if (inputCacheIdxSet.find(inputIdx) != inputCacheIdxSet.end()) {
            // Is KV cache
            const auto ringOffsetBytes = mRingBufferCacheManager.getRingOffset();
            return {buffer + ringOffsetBytes, size};
        }
        return {buffer, size}; // Not KV cache
    };
    ...
    for (int i = 0; i < getNumInputs(); i++) {
        ...
        const auto [buffer, size] = getBufferData(i);
