...
get_prompt_and_tokens(const std::string& inputString, const TokenizerUPtr& tokenizer,
    ...
    LOG(INFO) << "Num prompt tokens: " << inputTokens.size();
...
void llm_init(void** llmRuntime, const std::string& yamlConfigPath,
    ...
    if (preloadSharedWeights) {
        if (sharedWeightsHandle == nullptr) {
            Timer preloadTimer;
            preloadTimer.start();
            mtk_llm_shared_weights_preload(&sharedWeightsHandle, llmRuntimeOpt);
            LOG(INFO) << "Preload shared weights took: " << preloadTimer.reset() * 1000 << " ms";
        } else {
            LOG(INFO) << "Reusing preloaded shared weights.";
        }
    }
...
TokenType llm_digest_prompt(void* llmRuntime, const TokenizerUPtr& tokenizer,
    ...
    // Warn cache overflow
    const bool useCacheComp = mtk_llm_is_enabled_cache_eviction(llmRuntime);
    if (!useCacheComp && startTokenIndex + inputTokenCount > llmModelOpt.cacheSize) {
...
std::vector<TokenType> llm_gen_response(void* llmRuntime, const TokenizerUPtr& tokenizer,
    ...
    while (genTokCount < maxResponse && sequenceLength < maxTokenLength) {
        ...
        // Warn cache overflow
        if (!useCacheComp && sequenceLength == llmModelOpt.cacheSize) {
...
void llm_release_shared_weights() {
    if (sharedWeightsHandle == nullptr) {
        return;
    }
    mtk_llm_shared_weights_release_handle(sharedWeightsHandle);
